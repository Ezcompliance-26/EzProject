...
var setHeightWidth = function () {
	...
	$('.page-wrapper').css('height', (height - 1 ));
	...
	if(width<=1007){
		...
		$('#chat_list_scroll').css('height', (height - 111));  /*155*/
	...
	//if (width <= 1007) {
	//    $('#chat_list_scroll').css('height', (height - 270));
	//    $('.fixed-tab-menu-sidebar-right .chat-content').css('height', (height - 279));
	//    $('.fixed-tab-menu-sidebar-right .set-height-wrap').css('height', (height - 219));

	//}
	//else {
	//    $('#chat_list_scroll').css('height', (height - 204));
	//    $('.fixed-tab-menu-sidebar-right .chat-content').css('height', (height - 213));
	//    $('.fixed-tab-menu-sidebar-right .set-height-wrap').css('height', (height - 153));
	//}
    /*Right Sidebar Scroll End*/
...
var AdminUX = function(){
	...
	var sidebarNavAnchor2 = '.fixed-sidebar-left .side-nav  li .collapse-level-1 li .collapse-level-2 li a';
	
	var sidebarNavAnchorC1 = '.fixed-sidebar-left .side-nav  li a.c1';

	$(document).on("click", sidebarNavAnchor2, function (e) {
	    $wrapper.removeClass('slide-nav-toggle');
	});
	$(document).on("click", sidebarNavAnchorC1, function (e) {
	    $wrapper.removeClass('slide-nav-toggle');
	});
	/*Panel Remove*/
    ...
    /*Search Box Remove*/
	$(document).on('click', '.close-panel2', function (e) {
	...
	$(document).on('click', '#btnSearch', function (e) {
	    ...
	    //$('#collapseinputbox').attr('class', 'collapse');
	    //$('#CollapseSearchTableList').attr('class', 'collapse in');
	...
	$(document).on('click', '#toggle-btn,#open_right_sidebar_tab_menu_box,#setting_panel_btn', function (e) {
	...
	$(document).on('click', '#toggle-btn', function (e) {
	    ...
	    $wrapper.removeClass('open-right-sidebar-tab-menu-box open-setting-panel').toggleClass('slide-nav-toggle');
		return false;
	});
	$(document).on('click', '#open_right_sidebar', function (e) {
	    $("#IframeUpperLayer").addClass("iframe-upper-layer");
	    $wrapper.toggleClass('open-right-sidebar').removeClass('open-setting-panel');
	    return false;

	});
	$(document).on('click', '#CloseNotificationList', function (e) {
	    $("#IframeUpperLayer").removeClass("iframe-upper-layer");
	    $wrapper.toggleClass('open-right-sidebar').removeClass('open-setting-panel');
	    return false;
	});

	$(document).on('click', '#open_right_sidebar_tab_menu_box', function (e) {
	    $("#IframeUpperLayer").addClass("iframe-upper-layer");
	    $wrapper.toggleClass('open-right-sidebar-tab-menu-box').removeClass('open-setting-panel');
	    return false;

	});
	$(document).on('click', '#CloseRecentOpenPages', function (e) {
	    $("#IframeUpperLayer").removeClass("iframe-upper-layer");
	    $wrapper.toggleClass('open-right-sidebar-tab-menu-box').removeClass('open-setting-panel');
	    return false;
	});
	
	$(document).on('click', '#IframeUpperLayer, .wrapper .fixed-sidebar-left, .wrapper .navbar-fixed-top', function (e) {
	    $("#IframeUpperLayer").removeClass("iframe-upper-layer");
	});
	...
	$(document).on('click', 'body', function (e) {
	    if ($(e.target).closest('.fixed-tab-menu-sidebar-right,.setting-panel').length > 0) {
	        return;
	    }
	    $('body > .wrapper').removeClass('open-right-sidebar-tab-menu-box open-setting-panel');
	    return;
	});
	...
	$(document).on("keypress", "#input_msg_send", function (e) {
	    if ((e.which == 13) && (!$(this).val().length == 0)) {
	        $('<li class="self mb-10"><div class="self-msg-wrap"><div class="msg block pull-right">' + $(this).val() + '<div class="msg-per-detail mt-5"><span class="msg-time txt-light">1:30 pm</span></div></div></div><div class="clearfix"></div></li>').insertAfter(".fixed-tab-menu-sidebar-right .chat-content  ul li:last-child");
	        $(this).val('');
	    } else if (e.which == 13) {
	        alert('Please type somthing!');
	    }
	...
	/*01-07-2019 Amit kashyap*/
	$(document).on("click", "#ConversationMemberBox .msg-list-body .SelectStudentThreadBox", function (e) {
	    $(".ConversationMainBox").addClass('MoveToThreadBox');
		return false;
	});
	$(document).on("click", "#ConversationSingleMemberBox .search-member-title .MoveToConversationMember", function (e) {
	    $(".ConversationMainBox").removeClass('MoveToThreadBox');
	    return false;
	});
	...
	$(document).on("click", ".fixed-tab-menu-sidebar-right  .chat-cmplt-wrap .chat-data", function (e) {
	    $(".fixed-tab-menu-sidebar-right .chat-cmplt-wrap").addClass('chat-box-slide');
	    return false;
	});
	$(document).on("click", ".fixed-tab-menu-sidebar-right  #goto_back", function (e) {
	    $(".fixed-tab-menu-sidebar-right  .chat-cmplt-wrap").removeClass('chat-box-slide');
	    return false;
	});
	...
	/*Slimscroll*/
	$('.nicescroll-bar').slimscroll({height:'100%',color: '#fff', disableFadeOut : true,borderRadius:5,size:'6px',alwaysVisible:false});
	$('.message-nicescroll-bar').slimscroll({ height: '229px', size: '4px', color: '#878787', disableFadeOut: true, borderRadius: 0 });
	$('.fee-Stu-List-scroll').slimscroll({ height: '225px', size: '4px', color: '#878787', disableFadeOut: true, borderRadius: 0 });
	$('.msg-box-list-scroll').slimscroll({ height: '580px', size: '4px', color: '#878787', disableFadeOut: true, borderRadius: 0 });
	$('.complain-msg-body-scorll').slimscroll({ height: '545px', size: '4px', color: '#878787', disableFadeOut: true, borderRadius: 0, });
    ...
    /*Fullscreen Search Box Js*/
	$(document).on("click", ".full-screen2", function (e) {
	    $(this).parents('.table-heading-box').toggleClass('full-screen2');
	    $(window).trigger('resize');
	    return false;
	});
...
var boxLayout = function () {
    if ((!$wrapper.hasClass("rtl-layout")) && ($wrapper.hasClass("wrapper-boxed")))
        $(".wrapper-boxed .fixed-tab-menu-sidebar-right ").css({ right: $wrapper.offset().left + 300 });
    else if ($wrapper.hasClass("wrapper-boxed rtl-layout"))
        $(".wrapper-boxed .fixed-tab-menu-sidebar-right").css({ left: $wrapper.offset().left });
}
boxLayout();	
