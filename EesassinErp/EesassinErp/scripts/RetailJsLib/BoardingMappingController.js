app.BoardingMappingController = function ($scope, $element, $filter, myService) {
    ...
    $scope.BindBoarding = function () {
        var getData = myService.methode('POST', ("../Retail/SearchClientOnboarding"), { "Action": 4 });
    ...
    $scope.SaveRecord = function () {
        ...
        if (isValidate()) {
            ...
            collectionobj.BoardingType = $scope.BoardingType;
            ...
            var getData = myService.methode('POST', ("../Retail/INUBoardingMapping"), JSON.stringify(collectionobj));
            getData.then(function (response) {
    ...
    $scope.deleteRecord = function () {
        ...
        collectionobj.Id = $scope.hfId;
        var getData = myService.methode('POST', "../Retail/INUBoardingMapping", '{obj:' + JSON.stringify(collectionobj) + '}');
        getData.then(function (response) {
            if (showMsgBox(response.data.Result)) {
                $scope.ClearControl(1);
            }
    ...
    $scope.started = function () {
        ...
        var getData = myService.methode('POST', ("../Retail/SearchBoardingMapping"), JSON.stringify(collectionobj));
        getData.then(function (response) {
            var tblheader =
                [
                    ...
                    { "HeaderText": "BoardingType", "HeaderValue": "BoardingType", "Width": "100%", "ShowColumn": "Yes", "ImageColumn": "No" },
                    { "HeaderText": "BoardingTypeId", "HeaderValue": "BoardingTypeId", "Width": "100%", "ShowColumn": "No", "ImageColumn": "No" },
