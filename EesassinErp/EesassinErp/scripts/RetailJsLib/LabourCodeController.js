app.LabourCodeController = function ($scope, $element, $filter, myService, $http, $sce) {
    ...
    //    var getData = myService.methode('POST', "../Retail/SearchLabourcodeCompliance", '{obj:' + JSON.stringify(collectionobj) + '}');
    ...
    $scope.openComplianceModal = function (row) {
        ...
        $scope.selectedCompliance = {
            ...
            Risk: row.Risk || 'N/A',
            DetailedCompliance: row.DetailedCompliance || 'N/A',
            ImpactEmployer: row.ImpactEmployer || 'N/A',
...
`;
    ...
    $scope.BindFORTILESSearch = function () {
        ...
        var getData = myService.methode('POST', "../Retail/SearchLc", '{obj:' + JSON.stringify(collectionobj) + '}');
        ...
        getData.then(function (response) {
            ...
            // Filters
            var compliedList = rows.filter(r => normalize(r.Act) === 'The Code on Social Security, 2020');
            var delayCompliedList = rows.filter(r => normalize(r.Act) === 'The Occupational Safety, Health and Working Conditions Code, 2020');
            var nonCompliedList = rows.filter(r => normalize(r.Act) === 'The Industrial Relations Code, 2020');
            var pendingList = rows.filter(r => normalize(r.Act) === 'The Code on Wages, 2019');
           ...
           // const total = $scope.totalcompliance || 1; // avoid division by 0
            const total =
                Number($scope.totalComplied || 0) +
                Number($scope.totalDelayComplied || 0) +
                Number($scope.totalNonComplied || 0) +
                Number($scope.totalPending || 0);
   ...
   /*  ==================== BIND SEARCH ====================*/
    ...
    $scope.updateTileCounts = function () {
        ...
        $scope.compliedDocs = rows.filter(r => normalize(r.Act) === 'The Code on Social Security, 2020');
        $scope.pendingDocs = rows.filter(r => normalize(r.Act) === 'The Occupational Safety, Health and Working Conditions Code, 2020');
        $scope.delayedDocs = rows.filter(r => normalize(r.Act) === 'The Industrial Relations Code, 2020');
        $scope.nonCompliedDocs = rows.filter(r => normalize(r.Act) === 'The Code on Wages, 2019');
    ...
    $scope.openModal = function (type) {
        ...
        else if (type === 'The Code on Social Security, 2020') {
            $scope.modalData = rows.filter(r => normalize(r.Act) === 'The Code on Social Security, 2020'); // सही
        }
        else if (type === 'The Occupational Safety, Health and Working Conditions Code, 2020') {
            $scope.modalData = rows.filter(r => normalize(r.Act) === 'The Occupational Safety, Health and Working Conditions Code, 2020');
        }
        else if (type === 'The Industrial Relations Code, 2020') {
            $scope.modalData = rows.filter(r => normalize(r.Act) === 'The Industrial Relations Code, 2020');
        }
        else if (type === 'The Code on Wages, 2019') {
            $scope.modalData = rows.filter(r => normalize(r.Act) === 'The Code on Wages, 2019');
        }
    ...
    $scope.AfterverifyRecord = function (row) {
        ...
        if (isValidate()) {
            ...
            $http.post("../Retail/IUDLabourCodeStatutory", formData, {
