app.LocationMasterController = function ($scope, $element, $filter, myService, $http) {
    ...
    $scope.PrintStoreList = function () {
        ...
        var storeList = $scope.FilteredStoreList.length ? $scope.FilteredStoreList : $scope.StoreList;
        ...
        printContents += '<tr>';
        printContents += '<th colspan="7" style="text-align:center;font-size:16px">' + $scope.MapUser + '</th>';
        printContents += '</tr>';
        ...
        var printWindow = window.open('', '', 'height=600,width=800');
        printWindow.document.write('<html><head><title>Store List</title>');
        printWindow.document.write('<style>table {font-family: Arial; font-size: 12px;} th, td {padding: 5px;}</style>');
        printWindow.document.write('</head><body>');
        printWindow.document.write(printContents);
        printWindow.document.write('</body></html>');
        printWindow.document.close();
    ...
    $scope.GetExcel = function () {
        ...
        var dataToExport = $scope.FilteredStoreList.length ? $scope.FilteredStoreList : $scope.StoreList;
    ...
    $scope.ResetReport = function () {
        ...
        $scope.SelectedMonth = '';
        $('#lmSearch').val('');
        $scope.RegionList = [...new Set($scope.StoreAllList.map(x => x.RegionName))];
        $scope.StateNameList = [...new Set($scope.StoreAllList.map(x => x.STATE_NM))];
        $scope.CityNameList = [...new Set($scope.StoreAllList.map(x => x.CITY_NAME))];
        $scope.StoreNameList = [...new Set($scope.StoreAllList.map(x => x.StoreName))];
        $scope.YearList = [...new Set($scope.StoreAllList.map(x => x.ProposedDate.split('-')[2]))];
        $scope.MonthList = [...new Set($scope.StoreAllList.map(x => x.ProposedDate.split('-')[1]))]; 
        $scope.FilteredStoreList = angular.copy($scope.StoreAllList);
    }
    ...
    //    $scope.StateNameList = [...new Set(
    //        $scope.StoreAllList
    //            .filter(x => x.RegionName == region)
    //            .map(x => x.STATE_NM)
    //    )];
    ...
    //    $scope.CityNameList = [...new Set(
    //        $scope.StoreAllList
    //            .filter(x => x.RegionName == $scope.SelectedRegion && x.STATE_NM == state)
    //            .map(x => x.CITY_NAME)
    //    )];
    ...
    //    $scope.StoreNameList = [...new Set(
    //        $scope.StoreAllList
    //            .filter(x =>
    //                x.RegionName == $scope.SelectedRegion &&
    //                x.STATE_NM == $scope.SelectedState &&
    //                x.CITY_NAME == city)
    //            .map(x => x.StoreName)
    //    )];
    ...
    //                x.RegionName == $scope.SelectedRegion &&
    //                x.STATE_NM == $scope.SelectedState &&
    //                x.CITY_NAME == $scope.SelectedCity &&
    //                x.StoreName == store)
    //            .map(x => x.ProposedDate.split('-')[2])
    //    )];
    ...
    //        $scope.StoreAllList
    //            .filter(x =>
    //                x.StoreName == $scope.SelectedStore &&
    //                x.ProposedDate.split('-')[2] == year)
    //            .map(x => x.ProposedDate.split('-')[1])
    //    )];
    ...
    $scope.ApplyFilters = function () {
        ...
        var data = $scope.StoreAllList.filter(function (x) {

            var dateParts = x.ProposedDate ? x.ProposedDate.split('-') : [];
            var month = dateParts[1];
            var year = dateParts[2];

            return (!$scope.SelectedRegion || x.RegionName == $scope.SelectedRegion) &&
                (!$scope.SelectedState || x.STATE_NM == $scope.SelectedState) &&
                (!$scope.SelectedCity || x.CITY_NAME == $scope.SelectedCity) &&
                (!$scope.SelectedStore || x.StoreName == $scope.SelectedStore) &&
                (!$scope.Selectedyear || year == $scope.Selectedyear) &&
                (!$scope.SelectedMonth || month == $scope.SelectedMonth);
        });

        $scope.FilteredStoreList = data;
         
        $scope.RegionList = [...new Set(data.map(x => x.RegionName))];
        $scope.StateNameList = [...new Set(data.map(x => x.STATE_NM))];
        $scope.CityNameList = [...new Set(data.map(x => x.CITY_NAME))];
        $scope.StoreNameList = [...new Set(data.map(x => x.StoreName))];
        $scope.YearList = [...new Set(data.map(x => x.ProposedDate.split('-')[2]))];
        $scope.MonthList = [...new Set(data.map(x => x.ProposedDate.split('-')[1]))];
    }
                    ...
                    success: function (response) {
                        ...
                        if (id == '-2') {
                            ...
                            if (showMsgBox('999', 'Save', 'Save Successfully Store  :' + parts[0].trim(), 'success', 'btn-success')) {
                                ...
                                //$scope.StoreMasterForm = false;
                                $scope.GetStoreMaster(1, 10);
                                if ($scope.ButtonName == 'Create Location') {
                                    $scope.FireEmail(5, $scope.StoreManagerEmail, 0);
                                }
                                else { $scope.FireEmail(26, $scope.StoreManagerEmail, 0); }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (showMsgBox('999', 'Save', 'Save Successfully', 'success', 'btn-success')) {
                    ...
                    $scope.FireEmail(28, $scope._LicenseId, $scope._StoreId)
                /*    document.getElementById("lmUploadBack").click();*/
    ...
    $scope.EditStore = function (StoreCode) {
        ...
        $scope.storeeditlist = [];
        $scope.storeeditlist = $filter('filter')($scope.StoreAllList, { 'StoreCode': StoreCode });
...
`);
            ...
            setTimeout(function () {
                ...
                /*    obj.ProposedDate = (new Date()).toISOString().split('T')[0];*/
                    var excelDate = $(td[3]).text().trim();

                    var d = new Date(excelDate);
                    obj.ProposedDate = d.getFullYear() + '-' +
    ...
    $scope.SaveRecord = function () {
        if ($scope.LeftStore <= 0 && $scope.ButtonName == 'Create Location') { showMsgBox('999', 'Alert', 'Store Limit exceed:' + $scope.LeftStore + ', contact to admin', 'warning', 'btn-warning'); return; }
            ...
            for (let i = 0; i < $scope.StoreMasterList.length; i++) {
                ...
                var validCModels = ["Central Kitchen", "Retail Outlet", "Satellite Kitchen", "Warehouse", "Office", "Factory"];
                ...
                if (!validCModels.includes($scope.StoreMasterList[i].Category)) {
                    showMsgBox(
                        ...
                        '. Allowed values are Central Kitchen, Retail Outlet, Satellite Kitchen, Warehouse,Office, Factory',
  ...
  $scope.onFileChange = function (element, row) {
    ...
    $scope.printDTable = function () {
        ...
        <html>
        <head>
            <title>Location Document Report</title>
            <style>
        ...
        <body>
            <div class="report-header">
                ...
                <div class="report-title">Location Document Report</div>
    ...
    $scope.exportToDCSV = function () {
        ...
        link.download = 'LocationDocument.csv';
