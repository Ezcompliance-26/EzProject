        ...
        if ($scope.LeftStore <= 0 && $scope.ButtonName == 'Create Location') { showMsgBox('999', 'Alert', 'Store Limit exceed:' + $scope.LeftStore + ', contact to admin', 'warning', 'btn-warning'); return; }
            ...
            for (let i = 0; i < $scope.StoreMasterList.length; i++) {
                if ($scope.StoreMasterList[i].CompleteAddress.includes(',')) {
                    showMsgBox('999', 'warning', 'Complete Address has [,] please remove it from Unit Name ' + $scope.StoreMasterList[i].StoreName, 'warning', 'btn-warning');
                    return;
                }
                if ($scope.StoreMasterList[i].StoreLocation == '') {
                    showMsgBox('999', 'warning', 'Location not valid in Unit Name ' + $scope.StoreMasterList[i].StoreName, 'warning', 'btn-warning');
                    return;
                }
